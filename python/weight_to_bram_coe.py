...
def convert_weights(
    ...
    return emitted, pad_in_ch, pad_out_ch
...
def main() -> None:
    ...
    for layer, tensor_name, tensor in weight_tensors:
        ...
        units, pad_in_ch, pad_out_ch = convert_weights(flat_values, in_ch, out_ch, kernel_h, kernel_w)
        words = bytes_to_words(units)
        write_coe(words, output_path)

        groups = npu_channel_groups(out_ch, pad_out_ch)
        ...
        print(f"     storage_oihw={pad_out_ch}x{pad_in_ch}x{kernel_h}x{kernel_w}")
