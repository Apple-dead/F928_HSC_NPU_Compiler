...
运行时 feature map 的空间占用按 NPU 写回后的存储尺寸规划。若某一阶段输出的单通道矩阵逻辑尺寸小于 8x8，NPU 会补零成 8x8；因此 `runtime_regions` 中的 `size_bytes`、后续 split 偏移和下一层输入 footprint 都按 8x8 计算。`shape_nchw` 保留逻辑尺寸，`storage_shape_nchw` 记录实际预留尺寸。
...
其中 `logical_conv_output_hw` / `logical_output_hw` 表示模型语义尺寸；`conv_output_hw` / `output_hw` 表示生成指令时使用的 NPU 可见尺寸，已经包含 8x8 最小运行时存储规则。
...
`build.sh` 适合 Linux、WSL、Git Bash 等 bash 环境；`build.bat` 适合原生 Windows `cmd` 环境。两个脚本使用相同的 `MODEL_NAME` / `MODEL_PY_NAME` 配置，并执行同一套编译流程：提取模型参数、生成 memory plan、生成指令、生成参数 COE、生成指令 COE、合并最终 `target/all.coe`。
