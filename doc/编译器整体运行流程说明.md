...
flowchart TD
    ...
    B --> B1[MODEL_NAME<br/>MODEL_PY_NAME<br/>IMAGE_PATH<br/>BIAS_MOVE_PATH]
    ...
    E1 --> E2[调用 operator/conv dsmp madd relu]
...
每个 group 都有自己的 weight、bias、conv output、madd output、relu output 地址偏移。
...
例如 `3 -> 12` 的卷积层，输入通道会从 3 补齐到 4，输出通道可以按 8 + 4 分成两个 group。
...
bias 不只是按通道写一次，而是展开成和输出 feature map 对应的 bias 矩阵。
...
则 bias 会按空间位置重复写入，使 MADD 阶段可以把卷积输出和 bias 矩阵逐元素相加。

如果输出通道不是 4 的倍数，也需要补齐。对于输出通道超过单次处理能力的层，bias 同样要按 group 记录偏移。

`data/bias_move.json` 用于记录或指定 bias 量化、搬移、缩放相关配置。
