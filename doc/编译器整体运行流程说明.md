...
本文档介绍当前 NPU 编译器从模型、图片和权重文件出发，最终生成 FPGA BRAM 初始化文件 `target/all.coe` 的整体流程。
...
- 将输入图像、weight、bias、instruction 分别转换成 `.coe`；
...
存放各个初始化区域单独生成的 COE 文件，例如：

```text
coe/image.coe
coe/layer1_weight.coe
coe/layer1_bias.coe
coe/layer2_weight.coe
coe/layer2_bias.coe
coe/instr.coe
```
...
flowchart TD
    ...
    B --> B1[MODEL_NAME<br/>MODEL_PY_NAME<br/>IMAGE_PATH]
    ...
    B1 --> F[image_to_bram_coe.py]
    F --> F1[读取 data/image.jpg]
    F1 --> F2[按 NPU 数据布局和 4 通道对齐]
    F2 --> F3[coe/image.coe]

    C2 --> G[weight_to_bram_coe.py]
    D3 --> G
    G --> G1[按每层 weight region 生成]
    G1 --> G2[coe/layerN_weight.coe]

    C2 --> H[bias_to_bram_coe.py]
    D3 --> H
    H --> H1[按每层 bias region 生成]
    H1 --> H2[coe/layerN_bias.coe]

    E5 --> I[instr_txt_to_bram_coe.py]
    I --> I1[机器码指令转 BRAM COE]
    I1 --> I2[coe/instr.coe]

    F3 --> J[merge.py --memory-plan]
    G2 --> J
    H2 --> J
    I2 --> J
    D3 --> J
    J --> J1[读取 init_regions 顺序]
    J1 --> J2[按 image / weight / bias / instr 顺序拼接]
...
- `IMAGE_SOURCE` 控制输入图像是否参与初始化区；
- `INFER_PARSE_MODE` 和 `INFER_PARSE_LAYER_LIMIT` 控制解析完整模型还是只解析前若干层。
...
```text
coe/layerN_weight.coe
```

该阶段根据 `memory_plan.json` 中的 layer weight region，逐层生成 weight COE。
...
- 输入通道按 4 对齐；
- weight COE 不为输出通道补齐写入虚拟 kernel；
- 输出通道超过单次处理能力时按 group 组织；
- 每个 group 内按 NPU 读取顺序排列 kernel 数据。

例如 `3 -> 12` 的卷积层，输入通道会从 3 补齐到 4，输出通道可以按 8 + 4 分成两个 group；若是 `3 -> 3`，weight COE 只写 3 个有效输出通道，每个输出通道内部补 1 个全 0 输入 kernel。
...
bias 按输出通道写一次，补 0 到 4 的倍数后，每个 bias 作为 signed int32 word 原样写入 COE，不再截位或展开成输出 feature map 矩阵。

NPU 在 CONV 阶段根据 RCONV 的 bias bit 自动读取紧跟卷积核后的 bias 数据并完成相加。`data/bias_move.json` 已废弃，bias COE 生成不再读取缩放配置。
...
- `weight_to_bram_coe.py` 用它决定需要生成哪些层的 weight COE；
- `bias_to_bram_coe.py` 用它决定需要生成哪些层的 bias COE；
...
4. 检查 `coe/`
   ...
   ```text
   image.coe 是否存在
   每层 layerN_weight.coe 是否存在
   每层 layerN_bias.coe 是否存在
   instr.coe 是否存在
   ```
...
构建阶段使用 `python/params_to_bram_coe.py` 生成 `coe/layerN_params.coe`。该文件将每个最多 8 输出通道 group 写成 `weight -> 对齐后的 bias`；最终合并从 `memory_plan.json` 的 `init_regions` 读取这些参数区。每层参数区的起始地址和总大小保持原 weight+bias 布局的值不变。