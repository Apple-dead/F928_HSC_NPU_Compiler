...
运行时输出 feature map 还存在空间尺寸的最小占位规则：每个输出通道的矩阵若小于 8x8，NPU 会将其补零到 8x8 后写到指定输出地址。编译器侧不生成这些补零数据，但必须在地址规划中按 `max(height, 8) * max(width, 8)` 计算运行时输出区域、group 偏移以及后续层输入 footprint。
...
对于输出通道不是 4 的倍数的情况，输出 feature map、bias 和运行时区域仍按 4 通道对齐；若输出矩阵小于 8x8，运行时区域还要按 8x8 最小空间尺寸占位。但 weight COE 中只包含有效输出通道对应的卷积核，不包含虚拟输出通道对应的全 0 kernel。
...
CONV 配置寄存器中的图像大小字段写入 NPU 可见 `feature_width / 8`。如果上一算子输出逻辑宽度小于 8，但 NPU 已按 8x8 写回，则这里的 `feature_width` 取 8。例如输入宽度为 256 时：
...
DSMP 配置寄存器中的 image size 写 NPU 可见 feature map 边长，不除以 8。对于小于 8x8 的运行时输入，该边长按 8 传入。
...
ReLU 比较特殊：ReLU 配置寄存器中的 image size 写 NPU 可见 feature map 边长，不除以 8。对于小于 8x8 的运行时输入，该边长按 8 传入。
